...
class VetMapper(
    ...
    fun toVet(vetDto: VetDto): Vet = Vet().apply {
        id = vetDto.id
    ...
    fun toVet(vetFieldsDto: VetFieldsDto): Vet = Vet().apply {
    ...
    fun toVetDto(vet: Vet): VetDto = VetDto().apply {
        id = vet.id
        firstName = vet.firstName
        lastName = vet.lastName
        setSpecialties(vet.getSpecialties().map { specialtyMapper.toSpecialtyDto(it) })
    }

    fun toVetDtos(vets: Collection<Vet>): Collection<VetDto> =
        vets.map { toVetDto(it) }
}
