...
class JdbcUserRepositoryImpl(dataSource: DataSource) : UserRepository {
    ...
    private val insertUser = SimpleJdbcInsert(dataSource).withTableName("users")
    ...
    private fun getByUsername(username: String): User {
        val params = hashMapOf<String, Any>("username" to username)
    ...
    private fun updateUserRoles(user: User) {
        val params = hashMapOf<String, Any>("username" to user.username!!)
